...
public class BussinessCenter {
	...
	public static void VideoCallControl(int dwEventType, int dwUserId,
		...
		if (null == anychat) {
			System.out.println("com.bairuitech is null");
		}else
	...
	public void onVideoCallReply(int dwUserId, int dwErrorCode, int dwFlags,
			int dwParam, String szUserStr, AnyChatCoreIml callback) {
		// 回复类型处理
	...
	public void onVideoCallStart(int dwUserId, int dwFlags, int dwParam,
			String szUserStr, QueueInfoEntity mApplication) {
		// 呼叫开始事件响应
		stopSessionMusic();
		mApplication.TargetUserId = (dwUserId);
		mApplication.RoomId = (dwParam);
		// 界面是在这里跳转的
